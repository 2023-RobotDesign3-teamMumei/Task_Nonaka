...
def main():
    ...
    LEAVE_Z = 0.20
    PICK_Z = 0.1
    PLACE_POSITION = Point(0.09, -0.3, 0.0)
    STOPPER_mid_pos= Point(0.14,-0.3,0.12)
    STOPPER_POSITION = Point(0.14,-0.3,0.05)
    YOSEX_pos = Point(0.009,-0.3,0.05)
    STOPPER_POSITION2 = Point(0.03,-0.20,0.10)
    YOSEY_pos = Point(0.03,-0.38,0.10)
    ...
    if OBJECT_NAME in gazebo_model_states.name:
        ...
        # 設置位置に移動する
        target_pose.position = PLACE_POSITION
        ...
        gripper.wait_for_result(rospy.Duration(1.0))
        # アーム移動位置上空に移動する
        target_pose.position =STOPPER_mid_pos
        q = quaternion_from_euler(-math.pi, 0.0, -math.pi/2.0)
        target_pose.orientation.x = q[0]
        target_pose.orientation.y = q[1]
        target_pose.orientation.z = q[2]
        ...
        # アーム移動位置に移動する
        target_pose.position = STOPPER_POSITION
        q = quaternion_from_euler(-math.pi, 0.0, -math.pi/2.0)
        target_pose.orientation.x = q[0]
        target_pose.orientation.y = q[1]
        target_pose.orientation.z = q[2]
  	...
  	# アーム移動位置に移動する
        target_pose.position = YOSEX_pos
        q = quaternion_from_euler(-math.pi, 0.0, -math.pi/2.0)
        target_pose.orientation.x = q[0]
        target_pose.orientation.y = q[1]
        target_pose.orientation.z = q[2]
    ...
    # アーム移動位置に移動する
        target_pose.position = STOPPER_POSITION2
        q = quaternion_from_euler(-math.pi, 0.0, -math.pi/2.0)
        target_pose.orientation.x = q[0]
        target_pose.orientation.y = q[1]
        target_pose.orientation.z = q[2]
        ...
        # アーム移動位置に移動する
        target_pose.position = YOSEY_pos
        q = quaternion_from_euler(-math.pi, 0.0, -math.pi/2.0)
        target_pose.orientation.x = q[0]
        target_pose.orientation.y = q[1]
        target_pose.orientation.z = q[2]
